        ...
        for g, gl, i in zip(groups, groups_limits, range(0, num_groups)):
            ...
            while (sum(g[1]) < gl[1] and vertex_inserted):
                ...
                if (sum(g[1]) + vertices_weights[rcl[random_candidate][0]] <= gl[1]
                        ...
                        and not in_another_group(groups, i, rcl[random_candidate][0])):
                    g[0].append(rcl[random_candidate][0])
                    g[1].append(vertices_weights[rcl[random_candidate][0]])
                    current_value = current_value + rcl[random_candidate][1]
                else:
...
def local_search(groups, edges, groups_limits):
    ...
    while(improved):
        ...
        for i in range(100):
            ...
            modified_groups[insert_group][0].append(groups[remove_group][0][random_vertex])
            modified_groups[insert_group][1].append(groups[remove_group][1][random_vertex])

            del modified_groups[remove_group][0][random_vertex]
            del modified_groups[remove_group][1][random_vertex]
            if (sum(modified_groups[remove_group][1]) >= groups_limits[remove_group][0] 
                    and sum(modified_groups[insert_group][1]) <= groups_limits[insert_group][1]):
                new_score = checking(modified_groups, edges)
